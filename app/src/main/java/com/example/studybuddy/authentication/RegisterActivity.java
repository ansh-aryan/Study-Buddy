package com.example.studybuddy.authentication;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        roleButton = findViewById(R.id.registerActivityRegisterButtonID);
        registerForContextMenu(roleButton);
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo){
        ...
        menu.setHeaderTitle("Choose Role : ");
        //amenu.add(0,v.getId(),0,"Admin");
        menu.add(0,v.getId(),0,"Teacher");
        menu.add(0,v.getId(),0,"Student");
        menu.add(0,v.getId(),0,"Guest");
    }

    public boolean onContextItemSelected(MenuItem item){
        if(item.getTitle()=="Admin"){
            role = "Admin";
            roleButton.setText(role);
        }else if(item.getTitle()=="Teacher"){
            role = "Teacher";
            roleButton.setText(role);
        }else if(item.getTitle()=="Student"){
            role = "Student";
            roleButton.setText(role);
        }else if(item.getTitle()=="Guest"){
            role = "Guest";
            roleButton.setText(role);
        }
        return true;
    ...
    private void registerUser() {
        ...
        mAuth.createUserWithEmailAndPassword(email, pwd).addOnCompleteListener(this, task -> {
            if (task.isSuccessful()) {
                ...
                User user = new User(mAuth.getCurrentUser().getUid(), rid, email, name, phone, imageUrl, role, "true", currentDateAndTime);
